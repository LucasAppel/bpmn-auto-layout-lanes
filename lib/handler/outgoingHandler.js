...
export default {
  'addToGrid': ({ element, grid, visited, stack, laneMap }) => {
    let nextElements = [];
    ...
    const hasLaneMap = laneMap && laneMap.size > 0;
    ...
    outgoing.forEach((nextElement, index, arr) => {
      ...
      const laneKey = hasLaneMap ? laneMap.get(nextElement) : null;
      const previousInLane = hasLaneMap ? previousByLane.get(laneKey) : null;

      if (!previousElement || (hasLaneMap && !previousInLane)) {
        grid.addAfter(element, nextElement);
      }
      else if (is(element, 'bpmn:ExclusiveGateway') && is(nextElement, 'bpmn:ExclusiveGateway')) {
        grid.addAfter(previousInLane || previousElement, nextElement);
      }
      else {
        grid.addBelow(previousInLane || arr[index - 1], nextElement);
      }
      ...
      if (hasLaneMap) {
        previousByLane.set(laneKey, nextElement);
      }
