import { BpmnModdle } from 'bpmn-moddle';
import { isBoundaryEvent, isConnection } from './utils/elementUtils.js';
import {
  connectTextAnnotationToTask,
  DEFAULT_CELL_HEIGHT,
  DEFAULT_CELL_WIDTH,
  getTextAnnotationBounds,
  LANE_EXTRA_HEIGHT
} from './utils/layoutUtil.js';
import { Grid } from './Grid.js';
import { DiFactory } from './di/DiFactory.js';
import { is, getDefaultSize } from './di/DiUtil.js';
import { handlers } from './handler/index.js';
import { isFunction } from 'min-dash';

export class Layouter {
  constructor() {
    this.moddle = new BpmnModdle();
    this.diFactory = new DiFactory(this.moddle);
    this._handlers = handlers;
  }

  handle(operation, options) {
    return this._handlers
      .filter(handler => isFunction(handler[operation]))
      .map(handler => handler[operation](options));

  }

  async layoutProcess(xml) {
    const moddleObj = await this.moddle.fromXML(xml);
    const { rootElement } = moddleObj;

    this.diagram = rootElement;

    const firstRootProcess = this.getProcess();

    if (firstRootProcess) {

      this.setExpandedPropertyToModdleElements(moddleObj);

      this.setExecutedProcesses(firstRootProcess);

      this.createGridsForProcesses();

      this.cleanDi();

      this.createRootDi(firstRootProcess);

      this.drawProcesses();
    }

    return (await this.moddle.toXML(this.diagram, { format: true })).xml;
  }

  createGridsForProcesses() {
    const processes = this.layoutedProcesses.sort((a, b) => b.level - a.level);

    // create and add grids for each process
    // root processes should be processed last for element expanding
    for (const process of processes) {
      process.itSystemGroups = this.collectITSystemGroups(process);
      process.renderableITSystemGroups = [];

      // add base grid with collapsed elements
      process.grid = this.createGridLayout(process);

      expandGridHorizontally(process.grid);
      expandGridVertically(process.grid);

      this.applyLaneLayout(process);
      this.expandGridForITSystems(process);

      if (process.isExpanded) {
        const [ rowCount, colCount ] = process.grid.getGridDimensions();
        if (rowCount === 0) process.grid.createRow();
        if (colCount === 0) process.grid.createCol();
      }

    }
  }

  setExpandedPropertyToModdleElements(bpmnModel) {
    const allElements = bpmnModel.elementsById;
    if (allElements) {
      for (const element of Object.values(allElements)) {
        if (element.$type === 'bpmndi:BPMNShape' && element.isExpanded === true) element.bpmnElement.isExpanded = true;
      }
    }
  }

  setExecutedProcesses(firstRootProcess) {
    this.layoutedProcesses = [];

    const executionStack = [ firstRootProcess ];

    while (executionStack.length > 0) {
      const executedProcess = executionStack.pop();
      this.layoutedProcesses.push(executedProcess);
      executedProcess.level = executedProcess.$parent === this.diagram ? 0 : executedProcess.$parent.level + 1;

      const nextProcesses = executedProcess.flowElements?.filter(flowElement => flowElement.$type === 'bpmn:SubProcess') || [];

      executionStack.splice(executionStack.length, 0, ...nextProcesses);
    }
  }

  cleanDi() {
    this.diagram.diagrams = [];
  }

  createGridLayout(root) {
    const grid = new Grid();

    const flowElements = root.flowElements || [];
    const elements = flowElements.filter(el => !is(el,'bpmn:SequenceFlow'));

    const lanes = this.getProcessLanes(root);
    const laneMap = lanes.length ? this.mapElementsToLanes(lanes) : null;

    // check for empty process/subprocess
    if (!flowElements) {
      return grid;
    }

    bindBoundaryEventsWithHosts (flowElements);

    // Depth-first-search
    const visited = new Set();
    while (visited.size < elements.filter(element => !element.attachedToRef).length) {
      const startingElements = flowElements.filter(el => {
        return !isConnection(el) &&
            !isBoundaryEvent(el) &&
            (!el.incoming || !hasOtherIncoming(el)) &&
            !visited.has(el);
      });

      const stack = [ ...startingElements ];

      startingElements.forEach(el => {
        grid.add(el);
        visited.add(el);
      });

      this.handleGrid(grid, visited, stack, laneMap);

      if (grid.getElementsTotal() !== elements.length) {
        const gridElements = grid.getAllElements();
        const missingElements = elements.filter(el => !gridElements.includes(el) && !isBoundaryEvent(el));
        if (missingElements.length > 0) {
          stack.push(missingElements[0]);
          grid.add(missingElements[0]);
          visited.add(missingElements[0]);
          this.handleGrid(grid, visited, stack, laneMap);
        }
      }
    }
    return grid;
  }

  generateDi(layoutGrid , shift, procDi, process) {
    const diFactory = this.diFactory;

    const prePlaneElement = procDi ? procDi : this.diagram.diagrams[0];

    const planeElement = prePlaneElement.plane.get('planeElement');

    if (process) {
      const laneDis = this.createLaneDis(process, layoutGrid, shift);
      planeElement.push(...laneDis);
    }

    // Step 1: Create DI for all elements
    layoutGrid.elementsByPosition().forEach(({ element, row, col }) => {
      const laneShiftY = process ? this.getLaneShiftY(process, element) : 0;
      const elementShift = laneShiftY ? { x: shift.x, y: shift.y + laneShiftY } : shift;
      const dis = this
        .handle('createElementDi', { element, row, col, layoutGrid, diFactory, shift: elementShift })
        .flat();

      planeElement.push(...dis);
    });

    const artifactDis = process ? this.createITSystemDis(process) : null;

    if (artifactDis) {
      planeElement.push(...artifactDis.shapes);
    }

    // Step 2: Create DI for all connections
    layoutGrid.elementsByPosition().forEach(({ element, row, col }) => {
      const dis = this
        .handle('createConnectionDi', { element, row, col, layoutGrid, diFactory, shift })
        .flat();

      planeElement.push(...dis);
    });

    if (artifactDis) {
      planeElement.push(...artifactDis.edges);
    }
  }

  handleGrid(grid, visited, stack, laneMap) {
    while (stack.length > 0) {
      const currentElement = stack.pop();

      const nextElements = this.handle('addToGrid', { element: currentElement, grid, visited, stack, laneMap });

      nextElements.flat().forEach(el => {
        stack.push(el);
        visited.add(el);
      });
    }
  }

  getProcess() {
    return this.diagram.get('rootElements').find(el => el.$type === 'bpmn:Process');
  }

  createRootDi(processes) {
    this.createProcessDi(processes);
  }

  createProcessDi(element) {
    const diFactory = this.diFactory;

    const planeDi = diFactory.createDiPlane({
      id: 'BPMNPlane_' + element.id,
      bpmnElement: element
    });
    const diagramDi = diFactory.createDiDiagram({
      id: 'BPMNDiagram_' + element.id,
      plane: planeDi
    });

    const diagram = this.diagram;

    diagram.diagrams.push(diagramDi);

    return diagramDi;
  }

  /**
   * Draw processes.
   * Root processes should be processed first for element expanding
   */
  drawProcesses() {
    const sortedProcesses = this.layoutedProcesses.sort((a, b) => a.level - b.level);

    for (const process of sortedProcesses) {

      // draw processes in expanded elements
      if (process.isExpanded) {
        const baseProcDi = this.getElementDi(process);
        const diagram = this.getProcDi(baseProcDi);
        let { x, y } = baseProcDi.bounds;
        const { width, height } = getDefaultSize(process);
        x += DEFAULT_CELL_WIDTH / 2 - width / 4;
        y += DEFAULT_CELL_HEIGHT - height - height / 4;
        this.generateDi(process.grid, { x, y }, diagram, process);
        continue;
      }

      // draw other processes
      const diagram = this.diagram.diagrams.find(diagram => diagram.plane.bpmnElement === process);
      this.generateDi(process.grid, { x: 0, y: 0 }, diagram, process);
    }
  }

  applyLaneLayout(process) {
    const lanes = this.getProcessLanes(process);
    if (lanes.length === 0 || !process.grid) {
      return;
    }

    const elementLaneMap = this.mapElementsToLanes(lanes);
    const renderableITSystemGroups = (process.itSystemGroups || []).filter(({ host }) => elementLaneMap.has(host));
    const result = remapGridRowsByLanes(process.grid, lanes, elementLaneMap, renderableITSystemGroups);

    process.grid = result.grid;
    process.laneRows = result.laneRows;
    process.laneMap = elementLaneMap;
    process.laneIndexMap = new Map(result.laneRows.map((laneRow, index) => [ laneRow.lane, index ]));
    process.renderableITSystemGroups = renderableITSystemGroups;
  }

  expandGridForITSystems(process) {
    const { grid } = process;
    const groups = process.renderableITSystemGroups || [];

    if (!grid || groups.length === 0) {
      return;
    }

    const expandableGroups = groups
      .filter(({ annotations }) => annotations.length > 1)
      .sort((a, b) => {
        const [ rowA, colA ] = grid.find(a.host);
        const [ rowB, colB ] = grid.find(b.host);

        if (colA === colB) {
          return rowA - rowB;
        }

        return colA - colB;
      });

    expandableGroups.forEach(({ host, annotations }) => {
      const [ row, col ] = grid.find(host);

      if (row < 0 || col < 0) {
        return;
      }

      const extraColumns = annotations.length - 1;
      const insertions = getRequiredArtifactColumns(grid, row, col, extraColumns);

      if (insertions > 0) {
        grid.createCol(col, insertions);
      }
    });
  }

  getProcessLanes(process) {
    const laneSets = process.laneSets || [];
    if (laneSets.length === 0) {
      return [];
    }

    const firstLaneSet = laneSets[0];
    const lanes = firstLaneSet.lanes || [];

    return lanes;
  }

  mapElementsToLanes(lanes) {
    const elementLaneMap = new Map();

    lanes.forEach(lane => {
      const flowNodes = lane.flowNodeRef || [];

      flowNodes.forEach(node => {
        elementLaneMap.set(node, lane);

        const attachers = node.attachers || [];
        attachers.forEach(attacher => {
          elementLaneMap.set(attacher, lane);
        });
      });
    });

    return elementLaneMap;
  }

  collectITSystemGroups(root) {
    const artifacts = root.artifacts || [];
    const associations = artifacts.filter(artifact => is(artifact, 'bpmn:Association'));
    const groupsByHost = new Map();

    artifacts.forEach(annotation => {
      if (!isSupportedITSystemAnnotation(annotation)) {
        return;
      }

      const association = associations.find(candidate => {
        return candidate.sourceRef === annotation || candidate.targetRef === annotation;
      });

      if (!association) {
        return;
      }

      const host = association.sourceRef === annotation ? association.targetRef : association.sourceRef;

      if (!host || !is(host, 'bpmn:Task')) {
        return;
      }

      let group = groupsByHost.get(host);

      if (!group) {
        group = {
          host,
          annotations: []
        };

        groupsByHost.set(host, group);
      }

      group.annotations.push({
        annotation,
        association
      });
    });

    return Array.from(groupsByHost.values());
  }

  createLaneDis(process, layoutGrid, shift) {
    const diFactory = this.diFactory;
    const laneRows = process.laneRows || [];

    if (laneRows.length === 0) {
      return [];
    }

    const [ , colCount ] = layoutGrid.getGridDimensions();
    const width = Math.max(colCount, 1) * DEFAULT_CELL_WIDTH;

    return laneRows.map(({ lane, startRow, rowCount }, laneIndex) => {
      const laneOffset = laneIndex * LANE_EXTRA_HEIGHT;
      const bounds = {
        x: shift.x,
        y: shift.y + startRow * DEFAULT_CELL_HEIGHT + laneOffset - LANE_EXTRA_HEIGHT / 2,
        width,
        height: Math.max(rowCount, 1) * DEFAULT_CELL_HEIGHT + LANE_EXTRA_HEIGHT
      };

      return diFactory.createDiShape(lane, bounds, {
        id: lane.id + '_di',
        isHorizontal: true
      });
    });
  }

  getElementDi(element) {
    return this.diagram.diagrams
      .map(diagram => diagram.plane.planeElement).flat()
      .find(item => item.bpmnElement === element);
  }

  getProcDi(element) {
    return this.diagram.diagrams.find(diagram => diagram.plane.planeElement.includes(element));
  }

  createITSystemDis(process) {
    const diFactory = this.diFactory;
    const groups = process.renderableITSystemGroups || [];
    const shapes = [];
    const edges = [];

    groups.forEach(({ host, annotations }) => {
      const hostBounds = host.di?.bounds;

      if (!hostBounds) {
        return;
      }

      annotations.forEach(({ annotation, association }, index) => {
        const bounds = getTextAnnotationBounds(hostBounds, annotation, index);
        const shapeDi = diFactory.createDiShape(annotation, bounds, {
          id: annotation.id + '_di'
        });

        annotation.di = shapeDi;
        annotation.gridPosition = {
          row: host.gridPosition.row - 1,
          col: host.gridPosition.col + index
        };

        shapes.push(shapeDi);

        const path = connectTextAnnotationToTask(bounds, hostBounds);
        const waypoints = association.sourceRef === annotation ? path : [ ...path ].reverse();

        edges.push(diFactory.createDiEdge(association, waypoints, {
          id: association.id + '_di'
        }));
      });
    });

    return { shapes, edges };
  }

  getLaneShiftY(process, element) {
    if (!process?.laneMap || !process?.laneIndexMap) {
      return 0;
    }

    const lane = process.laneMap.get(element);
    if (!lane) {
      return 0;
    }

    const laneIndex = process.laneIndexMap.get(lane);
    if (!Number.isInteger(laneIndex)) {
      return 0;
    }

    return laneIndex * LANE_EXTRA_HEIGHT;
  }
}

export function bindBoundaryEventsWithHosts(elements) {
  const boundaryEvents = elements.filter(element => isBoundaryEvent(element));
  boundaryEvents.forEach(boundaryEvent => {
    const attachedTask = boundaryEvent.attachedToRef;
    if (!attachedTask) {
      return;
    }

    const attachers = attachedTask.attachers || [];
    attachers.push(boundaryEvent);
    attachedTask.attachers = attachers;
  });
}

/**
 * Check grid by columns.
 * If column has elements with isExpanded === true,
 * find the maximum size of elements grids and expand the parent grid horizontally.
 * @param grid
 */
function expandGridHorizontally(grid) {
  const [ numRows , maxCols ] = grid.getGridDimensions();
  for (let i = maxCols - 1 ; i >= 0; i--) {
    const elementsInCol = [];
    for (let j = 0; j < numRows; j++) {
      const candidate = grid.get(j, i);
      if (candidate && candidate.isExpanded) elementsInCol.push(candidate);
    }

    if (elementsInCol.length === 0) continue;

    const maxColCount = elementsInCol.reduce((acc,cur) => {
      const [ ,curCols ] = cur.grid.getGridDimensions();
      if (acc === undefined || curCols > acc) return curCols;
      return acc;
    }, undefined);

    const shift = !maxColCount ? 2 : maxColCount;
    grid.createCol(i, shift);
  }
}

/**
 * Check grid by rows.
 * If row has elements with isExpanded === true,
 * find the maximum size of elements grids and expand the parent grid vertically.
 * @param grid
 */
function expandGridVertically(grid) {
  const [ numRows , maxCols ] = grid.getGridDimensions();

  for (let i = numRows - 1 ; i >= 0; i--) {
    const elementsInRow = [];
    for (let j = 0; j < maxCols; j++) {
      const candidate = grid.get(i, j);
      if (candidate && candidate.isExpanded) elementsInRow.push(candidate);
    }

    if (elementsInRow.length === 0) continue;

    const maxRowCount = elementsInRow.reduce((acc,cur) => {
      const [ curRows ] = cur.grid.getGridDimensions();
      if (acc === undefined || curRows > acc) return curRows;
      return acc;
    }, undefined);

    const shift = !maxRowCount ? 1 : maxRowCount;

    // expand the parent grid vertically
    for (let index = 0; index < shift; index++) {
      grid.createRow(i);
    }
  }
}

function hasOtherIncoming(element) {
  const fromHost = element.incoming?.filter(edge => {
    const sourceRef = edge.sourceRef;
    return sourceRef && sourceRef !== element && sourceRef.attachedToRef === undefined;
  }) || [];

  const fromAttached = element.incoming?.filter(edge => {
    const sourceRef = edge.sourceRef;
    return sourceRef && sourceRef !== element && sourceRef.attachedToRef !== element;
  }) || [];

  return fromHost.length > 0 || fromAttached.length > 0;
}

function remapGridRowsByLanes(grid, lanes, elementLaneMap, artifactGroups = []) {
  const elements = grid.elementsByPosition();
  const laneRows = [];
  const laneOffsets = new Map();
  const laneRowMaps = new Map();
  const laneArtifactRows = new Map();
  let currentOffset = 0;

  artifactGroups.forEach(({ host }) => {
    const lane = elementLaneMap.get(host);
    const [ row ] = grid.find(host);

    if (!lane || row < 0) {
      return;
    }

    let rows = laneArtifactRows.get(lane);

    if (!rows) {
      rows = new Set();
      laneArtifactRows.set(lane, rows);
    }

    rows.add(row);
  });

  lanes.forEach(lane => {
    const rows = elements
      .filter(({ element }) => elementLaneMap.get(element) === lane)
      .map(({ row }) => row);

    const uniqueRows = Array.from(new Set(rows)).sort((a, b) => a - b);
    const rowMap = new Map();
    const artifactRows = laneArtifactRows.get(lane) || new Set();
    let visualRow = 0;

    uniqueRows.forEach(row => {
      if (artifactRows.has(row)) {
        visualRow += 1;
      }

      rowMap.set(row, visualRow);
      visualRow += 1;
    });

    const rowCount = visualRow > 0 ? visualRow : 1;

    laneOffsets.set(lane, currentOffset);
    laneRowMaps.set(lane, rowMap);
    laneRows.push({ lane, startRow: currentOffset, rowCount });

    currentOffset += rowCount;
  });

  const fallbackLane = lanes[0];
  const fallbackRowMap = laneRowMaps.get(fallbackLane) || new Map();
  const fallbackOffset = laneOffsets.get(fallbackLane) || 0;

  const newGrid = new Grid();

  elements.forEach(({ element, row, col }) => {
    const lane = elementLaneMap.get(element) || fallbackLane;
    const rowMap = laneRowMaps.get(lane) || fallbackRowMap;
    const laneOffset = laneOffsets.get(lane) || fallbackOffset;
    const mappedRow = rowMap.has(row) ? rowMap.get(row) : 0;
    const newRow = laneOffset + mappedRow;

    const [ existingRows ] = newGrid.getGridDimensions();
    if (newRow === 0 && col === 0 && existingRows === 0) {
      newGrid.add(element);
      return;
    }

    newGrid.add(element, [ newRow, col ]);
  });

  return { grid: newGrid, laneRows };
}

function isSupportedITSystemAnnotation(element) {
  if (!is(element, 'bpmn:TextAnnotation')) {
    return false;
  }

  const extensionValues = element.extensionElements?.values || [];

  return extensionValues.some(value => {
    const type = value.$type || '';
    const dataObjectType = value.dataObjectType || value.$attrs?.dataObjectType;

    return type === 'signavio:signavioType' && dataObjectType === 'IT-System';
  });
}

function getRequiredArtifactColumns(grid, row, col, extraColumns) {
  if (extraColumns <= 0) {
    return 0;
  }

  const [ , maxCols ] = grid.getGridDimensions();
  let boundaryCol = maxCols;

  for (let currentCol = col + 1; currentCol < maxCols; currentCol++) {
    if (grid.get(row, currentCol)) {
      boundaryCol = currentCol;
      break;
    }
  }

  const availableColumns = boundaryCol - col - 1;

  return Math.max(0, extraColumns - availableColumns);
}
