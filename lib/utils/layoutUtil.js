...
export const DEFAULT_CELL_WIDTH = 190;
export const DEFAULT_CELL_HEIGHT = 140;
export const LANE_EXTRA_HEIGHT = 30;
const TOP_ROUTE_CLEARANCE = 20;
...
export function connectElements(source, target, layoutGrid) {
  ...
  if (dX < 0) {
    ...
    if (sourceMid.y >= targetMid.y) {
      ...
      const minBelowY = Math.max(
        sourceBounds.y + sourceBounds.height + LANE_EXTRA_HEIGHT,
        targetBounds.y + targetBounds.height + LANE_EXTRA_HEIGHT
      );

      if (maxExpanded) {
        const routeY = !baseSourceGrid
          ? y + DEFAULT_CELL_HEIGHT + LANE_EXTRA_HEIGHT + maxExpanded * DEFAULT_CELL_HEIGHT
          : y + (baseSourceGrid.getGridDimensions()[0] + 1) * DEFAULT_CELL_HEIGHT;

        return [
          getDockingPoint(sourceMid, sourceBounds, 'b'),
          { x: sourceMid.x, y: Math.max(routeY, minBelowY) },
          { x: targetMid.x, y: Math.max(routeY, minBelowY) },
          getDockingPoint(targetMid, targetBounds, 'b')
        ];
      }

      const routeY = !baseSourceGrid
        ? y + DEFAULT_CELL_HEIGHT + LANE_EXTRA_HEIGHT
        : y + (baseSourceGrid.getGridDimensions()[0] + 1) * DEFAULT_CELL_HEIGHT;
  ...
  if (dY === 0) {
    if (isDirectPathBlocked(source, target, layoutGrid)) {
      const { y } = coordinatesToPosition(source.gridPosition.row, source.gridPosition.col);
      const routeY = !baseSourceGrid
        ? y + LANE_EXTRA_HEIGHT - TOP_ROUTE_CLEARANCE
        : y - (baseSourceGrid.getGridDimensions()[0] + 1) * DEFAULT_CELL_HEIGHT - TOP_ROUTE_CLEARANCE;

      // Route on top
      return [
        getDockingPoint(sourceMid, sourceBounds, 't'),
        { x: sourceMid.x, y: routeY },
        { x: targetMid.x, y: routeY },
        getDockingPoint(targetMid, targetBounds, 't')
      ];
    } else {
      ...
      if (baseSourceGrid) {
        firstPoint.y = sourceBounds.y + sourceBounds.height / 2;
      }

      if (baseTargetGrid) {
        lastPoint.y = targetBounds.y + targetBounds.height / 2;
      }
      return [
        firstPoint,
        lastPoint
...
export function getTextAnnotationBounds(hostBounds, annotation, index = 0) {
  const { width, height } = getDefaultSize(annotation);

  return {
    width,
    height,
    x: Math.round(hostBounds.x + index * DEFAULT_CELL_WIDTH),
    y: Math.round(hostBounds.y - DEFAULT_CELL_HEIGHT)
  };
}

export function connectTextAnnotationToTask(annotationBounds, taskBounds) {
  const source = getDockingPoint(getMid(annotationBounds), annotationBounds, 'b');
  const target = getDockingPoint(getMid(taskBounds), taskBounds, 't');
  ...
  const middleY = source.y + Math.round((target.y - source.y) / 2);
...
function directManhattanConnect(source, target, layoutGrid) {
  ...
  if (dY > 0) {
    ...
    // If above, calculate more waypoints
    return false;
